  ...
  const generateImage = async (text, negativeText) => {
    ...
    try {
      const response = await fetch(`${baseURL}/generate-image`, {
        ...
        body: JSON.stringify({ prompt: `${text}` , negative_prompt: `${negativeText}`}), // Concatenate image type with text
      });
  ...
  const generateImages = async (text, negativeText) => {
    ...
    switch (imageQuality) {
     ...
     const selections = [
      ...
      selectedWeather,
      selectedTechnique,
    ].filter(Boolean).join(', '); // Filters out any empty selections and joins the rest with a comma
...
const fullPrompt = selections.length > 0 ? `${text}, ${selections}` : text;
    try {
         ...
         for (let i = 0; i < selectedImageCount; i++) { // Loop to call the API four times
        const response = await fetch(`${baseURL}/generate-image`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ prompt: fullPrompt, negative_prompt: negativeText, num_inference_steps: num_inference_steps}),
        });
  ...
  const fetchSentimentData = async (text) => {
    ...
    try {
      ...
      if (data && data.sentiment_scores && Array.isArray(data.sentiment_scores[0])) {
...
/>
      ...
      <Prompt selectedFeature={selectedFeature} onGenerate={
          selectedFeature === 'imageGenerator'
            ? generateImages
            : selectedFeature === 'semanticAnalysis'
            ? fetchSentimentData: selectedFeature === 'contentAnalysis' ? fetchFiltrationScores
...
{selectedFeature === 'translation' && !isTranslating && (
  <div className="translation-container">
    ...
    <div className="translation-prompt">
      ...
      <div className="prompt-output" contentEditable="false">{translatedText}</div>
    </div>
    {/* {negativePrompt && (
      <div className="translation-prompt">
    ...
    {translatedNegativeText && (
      <div className="translation-prompt">
        ...
        <div className="prompt-output" contentEditable="false">{translatedNegativeText}</div>
      </div>
    )}
  </div>
)}

{selectedFeature === 'contentAnalysis' && filtrationScores?.length > 0 && (
